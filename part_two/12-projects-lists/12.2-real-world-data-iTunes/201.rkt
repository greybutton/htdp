...
(define (select-all-album-title/unique lt)
  (select/unique (list) lt))
  
(define (select/unique l lt)
  (cond
    [(empty? lt) l]
    [(member (track-album (first lt)) l)
     (select/unique l (rest lt))]
    [else (select/unique
           (append
            l
            (list (track-album (first lt))))
           (rest lt))]))

(check-expect
 (select-all-album-title/unique LTRACKS)
 (list (track-album BITCH-LASAGNA) (track-album WAKE-UP-CALL)))
